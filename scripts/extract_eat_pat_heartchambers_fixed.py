...
Spec5.3.mdとz-axis.mdのフィードバックに基づく改善：
...
# v5.4 改善パラメータ
DEFAULT_CONTACT_EPS_MM = 1.0      # 接触判定の等方膨張 [mm]
DEFAULT_LUNG_DOM_RATIO = 2.0      # 肺接触が腹部臓器より何倍優勢か（推奨値を1.5に調整）
DEFAULT_MIN_LUNG_TOUCH_VOX = 150  # 肺接触の最小ボクセル数（推奨値を150に調整）
DEFAULT_POST_TRIM_MM = 1.5        # 最終整形用の安全帯 [mm]
DEFAULT_SEED_Z_MARGIN_MM = 10.0   # シード生成のZ範囲マージン [mm]
DEFAULT_SEAL_DIAPHRAGM_MM = 4.0   # 横隔膜穴塞ぎ [mm]（2.0→3.0に強化）
DEFAULT_REQUIRE_SHELL_TOUCH = True # Shell接触を必須とするか

# Z軸範囲モード
Z_RANGE_MODES = ['rpa_to_diaphragm', 'rpa_to_apex', 'current_prox_vessel']
DEFAULT_Z_RANGE_MODE = 'rpa_to_diaphragm'  # 推奨: RPA→横隔膜モード
...
def load_heart_components(totalseg_dir, verbose=False):
    """
    心臓コンポーネントを読み込んで統合
    heartchambers_highresタスクの出力のみを使用（フォールバックなし）
    """
    totalseg_dir = Path(totalseg_dir)
    heart_mask = None
    affine = None
    
    # heartchambers_highresタスクの出力（heart_プレフィックス付き）
    required_components = [
        "heart_myocardium",
        "heart_atrium_left", 
        "heart_atrium_right",
    ...
    for component_name in required_components:
        component_path = totalseg_dir / f"{component_name}.nii.gz"
        if component_path.exists():
            img = nib.load(str(component_path))
    ...
    # 必須コンポーネントが見つからない場合はエラー
    if missing_components:
        raise FileNotFoundError(
            f"\n[ERROR] Required heart components not found!\n"
            f"Missing: {', '.join(missing_components)}\n"
            f"Please run TotalSegmentator with --task heartchambers_highres first.\n"
            f"Example:\n"
            f"  TotalSegmentator -i <ct.nii.gz> -o <output_dir> --task heartchambers_highres -l aca_DHDKRFJMHUX0PK"
        )
    
    if verbose:
        print(f"  Successfully loaded all {len(components_found)} heart components")
...
def compute_diaphragm_zmin(lungs_mask, heart_mask, spacing, 
                           seal_diaphragm_holes_mm=3.0, pad_mm=30.0, 
                           margin_slices=1, verbose=False):
    """
    横隔膜近似によるZ下限の計算
    
    Args:
        lungs_mask: 肺マスク（左右統合済み）
        heart_mask: 心臓マスク
        spacing: ボクセルサイズ [mm]
        seal_diaphragm_holes_mm: 横隔膜穴塞ぎ距離 [mm]
        pad_mm: 心臓周囲のXY範囲拡張 [mm]
        margin_slices: 横隔膜下のマージン（スライス数）
        verbose: 詳細ログ出力
    ...
    lungs_closed = binary_closing(lungs_mask, structure=struct, iterations=iterations)
    
    # 心臓のXY外接矩形を取得
    heart_any_xy = np.any(heart_mask, axis=2)
    y_idx, x_idx = np.where(heart_any_xy)
    
    if len(x_idx) == 0 or len(y_idx) == 0:
        # 心臓が見つからない場合は肺全体で判定
        z_has_lung = np.any(lungs_closed, axis=(0, 1))
    else:
        # 心臓周囲±pad_mmの範囲に限定
        pad_voxels_x = int(np.ceil(pad_mm / spacing[0]))
        pad_voxels_y = int(np.ceil(pad_mm / spacing[1]))
        
        x0 = max(0, x_idx.min() - pad_voxels_x)
        x1 = min(heart_mask.shape[0], x_idx.max() + pad_voxels_x + 1)
        y0 = max(0, y_idx.min() - pad_voxels_y)
        y1 = min(heart_mask.shape[1], y_idx.max() + pad_voxels_y + 1)
        
        # この範囲内で肺が存在する最尾側スライスを探す
        z_has_lung = np.any(lungs_closed[x0:x1, y0:y1, :], axis=(0, 1))
    
    # 肺が存在する最初のZ座標（最も尾側）
    z_lung_indices = np.where(z_has_lung)[0]
    if z_lung_indices.size == 0:
        # 肺が見つからない場合は心臓ベースで決定
        z_heart = np.any(heart_mask, axis=(0, 1))
        z_heart_indices = np.where(z_heart)[0]
        if z_heart_indices.size == 0:
            raise ValueError("Neither lungs nor heart found in masks")
        zmin = max(0, z_heart_indices[0] - margin_slices)
        if verbose:
            print(f"  WARNING: No lungs found, using heart-based zmin: {zmin}")
    else:
        # 横隔膜は肺の下端の少し下
        zmin = max(0, z_lung_indices[0] - margin_slices)
        if verbose:
            print(f"  Diaphragm-based zmin: {zmin}")
            print(f"    Lung lowest slice: {z_lung_indices[0]}")
            print(f"    Applied margin: {margin_slices} slices")
    
    return zmin

def compute_apex_zmin(heart_mask, margin_slices=2, verbose=False):
    """
    左室心尖によるZ下限の計算
    
    Args:
        heart_mask: 心臓マスク
        margin_slices: 心尖下のマージン（スライス数）
        verbose: 詳細ログ出力
    ...
    if verbose:
        print(f"  Apex-based zmin: {zmin}")
        print(f"    Heart lowest slice: {z_heart_indices[0]}")
        print(f"    Applied margin: {margin_slices} slices")
...
def extract_eat_pat_improved_v5_4(ct_path, totalseg_dir, output_dir, 
                                     ...
                                     # Z-axis range mode
                                     z_range_mode=DEFAULT_Z_RANGE_MODE,
                                     verbose=False):
    """
    改善版v5.4 EAT+PAT（心膜周囲脂肪）抽出 with Z軸処理改善
    接触判定と禁止帯の順序を入れ替え、シードZ制限、Shell接触要件を追加
    Z軸範囲モードの追加（RPA→横隔膜、RPA→心尖、現行モード）
    """
    ...
    # 7. Z軸範囲の決定（モードに応じて選択）
    print(f"\n8. Computing anatomical boundaries (mode: {z_range_mode})...")
    
    # 警告：設定の矛盾をチェック
    if abdomen_gap_mm >= contact_eps_mm:
        print(f"  WARNING: abdomen_gap_mm ({abdomen_gap_mm}) >= contact_eps_mm ({contact_eps_mm})")
        print(f"           This may cause abdominal organ contacts to be always zero!")
    ...
    # 下端はモードに応じて選択
    if z_range_mode == 'rpa_to_diaphragm':
        # 横隔膜ベースのZ下限（推奨）
        z_min = compute_diaphragm_zmin(
            lungs, heart_mask, spacing, 
            seal_diaphragm_holes_mm=seal_diaphragm_holes_mm,
            pad_mm=30.0, margin_slices=1, verbose=verbose
        )
        print(f"  Using diaphragm-based Z-range (RPA to diaphragm)")
    elif z_range_mode == 'rpa_to_apex':
        # 心尖ベースのZ下限
        z_min = compute_apex_zmin(
            heart_mask, margin_slices=2, verbose=verbose
        )
        print(f"  Using apex-based Z-range (RPA to apex)")
    else:  # 'current_prox_vessel'
        # 現行の血管近接ベースのZ下限
        z_min = compute_intrapericardial_zmin(
            totalseg_dir, heart_mask, spacing, 
            prox_mm=5.0, z_margin_slices=1, verbose=verbose
        )
        print(f"  Using proximal vessel-based Z-range (current method)")
    ...
    # ILAMコンポーネントを保存（v5.4: 可視化用）
    ilam_components_path = masks_dir / "ilam_components.nii.gz"
    ilam_components_img = nib.Nifti1Image(ilam_components.astype(np.uint8), ct_img.affine, ct_img.header)
    nib.save(ilam_components_img, str(ilam_components_path))
    print(f"  ILAM components saved: {ilam_components_path}")
    ...
    stats = {
        ...
        "parameters": {
            ...
            "processing_version": "v5.4 with Enhanced Z-axis Processing",
            "z_range_mode": z_range_mode,
            "ilam_settings": {
...
def main():
    parser = argparse.ArgumentParser(
        description="Improved EAT+PAT extraction v5.4 with Enhanced Z-axis Processing",
    ...
    parser.add_argument(
        "--z-range-mode",
        type=str,
        choices=Z_RANGE_MODES,
        default=DEFAULT_Z_RANGE_MODE,
        help=f"Z軸範囲の決定方法:\n"
             f"  rpa_to_diaphragm: RPA→横隔膜 (推奨、文献標準)\n"
             f"  rpa_to_apex: RPA→心尖\n"
             f"  current_prox_vessel: 現行の血管近接ベース\n"
             f"(default: {DEFAULT_Z_RANGE_MODE})"
    )
    ...
    try:
        ...
        stats = extract_eat_pat_improved_v5_4(
            ...
            z_range_mode=args.z_range_mode,
            verbose=args.verbose
        )
        
        print("\n" + "="*60)
        print("Improved EAT+PAT extraction v5.4 (Enhanced Z-axis Processing) completed successfully!")
