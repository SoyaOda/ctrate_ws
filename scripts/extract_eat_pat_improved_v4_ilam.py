#!/usr/bin/env python3
"""
改善版v4 EAT+PAT抽出：ILAM (Inferior Lung-Adjacency Mode) 実装
下部帯域で肺接触ベースの脂肪抽出を追加
lung_ROI_2.mdのフィードバックに基づく実装
"""
...
DEFAULT_INFERIOR_BAND_MM = 25.0  # 下部帯域の高さ [mm]
DEFAULT_LUNG_CONTACT_MM = 1.5    # 肺接触判定の許容距離 [mm] (δ)
DEFAULT_ABDOMEN_GAP_MM = 3.0     # 腹部臓器除外の膨張距離 [mm] (γ)
DEFAULT_MAX_HEART_DIST_MM = 30.0 # 心臓からの最大距離 [mm] (ρ)
...
def extract_eat_pat_improved_v4_ilam(ct_path, totalseg_dir, output_dir, 
                                     ...
                                     verbose=False):
    """
    改善版v4 EAT+PAT（心膜周囲脂肪）抽出 with ILAM
    下部帯域で肺接触ベースの脂肪抽出を追加
    """
    print("\n[Improved EAT+PAT Extraction v4 with ILAM]")
    print("="*60)
    ...
    # 14. ILAM (Inferior Lung-Adjacency Mode) の適用
    print("\n14. Applying ILAM (Inferior Lung-Adjacency Mode)...")
    print(f"  Parameters:")
    print(f"    - Inferior band: {inferior_band_mm} mm")
    print(f"    - Lung contact tolerance: {lung_contact_mm} mm (δ)")
    ...
    # mm をスライス数に変換するヘルパー関数
    def mm_to_iterations(mm_dist):
        return max(1, int(np.ceil(mm_dist / min_spacing)))
    
    # 下部帯域の計算
    band_slices = int(np.ceil(inferior_band_mm / min(spacing)))
    z_switch = min(z_min + band_slices, z_max)
    print(f"  ILAM Z-range: {z_min} to {z_switch} ({z_switch - z_min} slices)")
    
    # 肺膨張（接触判定用）
    lungs_dilated = binary_dilation(
        lungs, 
        structure=struct, 
        iterations=mm_to_iterations(lung_contact_mm)
    )
    
    # 腹部臓器の膨張（禁止帯）
    abdomen_forbidden = binary_dilation(
        abdominal_organs, 
        structure=struct, 
        iterations=mm_to_iterations(abdomen_gap_mm)
    )
    
    # 心臓からの距離制約
    dist_to_heart = distance_transform_edt(~heart_mask, sampling=spacing)
    near_heart = dist_to_heart <= max_heart_dist_mm
    
    # 肺接触候補 C2 の計算
    # C2 = torso_fat ∩ (~lungs) ∩ touch(torso_fat, lungs, δ) ∩ (dist_heart ≤ ρ) ∩ ~(dilate(liver|spleen|stomach, γ))
    cand_contact = (
        fat_clipped &           # torso_fat (Z-clipped)
        mediastinum &           # (~lungs) 縦隔内
        lungs_dilated &         # 肺接触判定
        near_heart &            # 心臓からの距離制約
        ~abdomen_forbidden      # 腹部臓器除外
    )
    ...
    if aorta_path.exists():
        ...
        # 遠位大動脈を2-3mm膨張して禁止帯を作成
        distal_aorta_forbidden = binary_dilation(
            distal_aorta,
            structure=struct,
            iterations=mm_to_iterations(2.5)
        )
        
        # 遠位大動脈周囲を除外
        cand_contact = cand_contact & ~distal_aorta_forbidden
        print(f"  Excluded distal aorta PVAT")
    
    # ILAMで追加された体積を報告
    ilam_voxels = np.sum(cand_contact)
    print(f"  ILAM candidate voxels: {ilam_voxels:,}")
    
    # 最終的なEAT+PAT: Shellベース ∪ ILAM候補
    if replace_by_contact:
        # 下部帯域では肺接触ベースに置換（実験用）
        z_mask = np.zeros_like(eat_pat_shell, dtype=bool)
        z_mask[:, :, z_switch+1:] = True
        pericardial_fat_mask = (eat_pat_shell & z_mask) | cand_contact
    else:
        # 加算モード（推奨）
        pericardial_fat_mask = eat_pat_shell | cand_contact
    ...
    print(f"  Added by ILAM: {added_by_ilam:,} voxels")
    print(f"  Combined EAT+PAT: {combined_voxels:,} voxels")
    ...
    stats = {
        ...
        "parameters": {
            ...
            "improvements": [
                ...
                "ILAM: Inferior Lung-Adjacency Mode for lower slices",
...
def main():
    parser = argparse.ArgumentParser(
        description="Improved EAT+PAT extraction v4 with ILAM (Inferior Lung-Adjacency Mode)",
    ...
    parser.add_argument(
        ...
        help=f"ILAM maximum distance from heart in mm (ρ) (default: {DEFAULT_MAX_HEART_DIST_MM})"
    )
    ...
    # 実行
    try:
        stats = extract_eat_pat_improved_v4_ilam(
            ...
            max_heart_dist_mm=args.max_heart_dist_mm,
            replace_by_contact=args.replace_by_contact,
            verbose=args.verbose
        )
        
        print("\n" + "="*60)
        print("Improved EAT+PAT extraction v4 with ILAM completed successfully!")
