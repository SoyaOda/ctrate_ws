...
def create_slice_visualization(ct_data, masks, slice_idx, spacing, output_path):
    ...
    if np.any(masks['eat_pat'][:, :, slice_idx]):
        ...
        ax3.imshow(eat_pat_masked, cmap='YlOrRd', alpha=0.6, vmin=0, vmax=1)
    
    eat_pat_pixels = np.sum(masks['eat_pat'][:, :, slice_idx])
    eat_pat_area = eat_pat_pixels * spacing[0] * spacing[1] / 100
    ax3.set_title(f'EAT+PAT (Final)\nArea: {eat_pat_area:.1f} cm²', fontsize=12, fontweight='bold')
    ax3.axis('off')
    
    # 4. CT + Visceral Fat (左下)
    ...
    if np.any(masks['visceral_fat'][:, :, slice_idx]):
        ...
        ax4.imshow(vf_masked, cmap='YlOrBr', alpha=0.5, vmin=0, vmax=1)
    ...
    if np.any(masks['heart'][:, :, slice_idx]):
        heart_masked = np.ma.masked_where(masks['heart'][:, :, slice_idx] < 0.5,
                                         masks['heart'][:, :, slice_idx])
        ax6.imshow(heart_masked, cmap='Reds', alpha=0.4, vmin=0, vmax=1)
...
def create_mip_visualization(ct_data, masks, spacing, output_path):
    ...
    stats_text += f"ILAM / total EAT+PAT:     {ilam_percent:5.1f}%\n"
    ...
    plt.suptitle('EAT+PAT v5 Analysis with ILAM - MIP Views', fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    plt.close()
