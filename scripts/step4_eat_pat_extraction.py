#!/usr/bin/env python3
"""
STEP4: EAT/PAT (Epicardial/Pericardial Adipose Tissue) 抽出
心筋マスクから膨張させた領域内の脂肪組織を抽出
"""

import sys
import os
import nibabel as nib
import numpy as np
from scipy.ndimage import binary_dilation, generate_binary_structure
from pathlib import Path

def extract_eat_pat(ct_path, myocardium_path, output_dir, verbose=True):
    """
    EAT/PAT抽出メイン処理
    
    Args:
        ct_path: 入力CT画像パス
        myocardium_path: 心筋セグメンテーションパス
        output_dir: 出力ディレクトリ
        verbose: 詳細出力フラグ
    """
    
    # 出力ディレクトリ作成
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    
    if verbose:
        print(f"読み込み中: {ct_path}")
    
    # CT画像と心筋マスク読み込み
    ct_nii = nib.load(ct_path)
    ct_data = ct_nii.get_fdata()
    
    if not os.path.exists(myocardium_path):
        print(f"警告: 心筋セグメンテーションが見つかりません: {myocardium_path}")
        print("代替として心臓全体マスクを使用します")
        # 代替パスを試す
        alt_paths = [
            myocardium_path.replace('heart_myocardium', 'heart'),
            myocardium_path.replace('step3_heartchambers', 'step3_optimized').replace('heart_myocardium', 'heart')
        ]
        for alt_path in alt_paths:
            if os.path.exists(alt_path):
                myocardium_path = alt_path
                print(f"使用: {alt_path}")
                break
    
    myocardium_nii = nib.load(myocardium_path)
    myocardium_mask = myocardium_nii.get_fdata() > 0
    
    # ボクセルサイズ取得
    spacing = ct_nii.header.get_zooms()[:3]
    voxel_volume_ml = np.prod(spacing) / 1000.0  # mm³ to ml
    
    if verbose:
        print(f"CT画像サイズ: {ct_data.shape}")
        print(f"ボクセルサイズ: {spacing} mm")
        print(f"ボクセル体積: {voxel_volume_ml:.6f} ml")
    
    # 脂肪組織の閾値設定（HU値: -190 ~ -30）
    fat_mask = (ct_data >= -190) & (ct_data <= -30)
    
    # 3D構造要素（26近傍）
    struct_elem = generate_binary_structure(3, 2)
    
    # 膨張距離計算（mm単位をボクセル数に変換）
    min_spacing = min(spacing)
    dilation_5mm = int(np.ceil(5.0 / min_spacing))   # EAT用: 5mm
    dilation_10mm = int(np.ceil(10.0 / min_spacing))  # PAT用: 10mm
    
    if verbose:
        print(f"膨張ステップ数: EAT={dilation_5mm}, PAT={dilation_10mm}")
    
    # EAT領域作成（心筋から5mm膨張）
    if verbose:
        print("EAT領域計算中...")
    eat_roi = myocardium_mask.copy()
    for i in range(dilation_5mm):
        eat_roi = binary_dilation(eat_roi, struct_elem)
        if verbose and (i+1) % 5 == 0:
            print(f"  膨張: {i+1}/{dilation_5mm}")
    
    # PAT領域作成（心筋から10mm膨張）
    if verbose:
        print("PAT領域計算中...")
    pat_roi = myocardium_mask.copy()
    for i in range(dilation_10mm):
        pat_roi = binary_dilation(pat_roi, struct_elem)
        if verbose and (i+1) % 5 == 0:
            print(f"  膨張: {i+1}/{dilation_10mm}")
    
    # EAT/PAT抽出（脂肪 AND 膨張領域 AND NOT 心筋）
    eat_mask = fat_mask & eat_roi & ~myocardium_mask
    pat_mask = fat_mask & pat_roi & ~myocardium_mask
    
    # 体積計算
    eat_volume_ml = np.sum(eat_mask) * voxel_volume_ml
    pat_volume_ml = np.sum(pat_mask) * voxel_volume_ml
    
    # 保存
    eat_path = os.path.join(output_dir, "eat_5mm.nii.gz")
    pat_path = os.path.join(output_dir, "pat_10mm.nii.gz")
    
    if verbose:
        print(f"保存中: {eat_path}")
    nib.save(nib.Nifti1Image(eat_mask.astype(np.uint8), 
                              ct_nii.affine, ct_nii.header), eat_path)
    
    if verbose:
        print(f"保存中: {pat_path}")
    nib.save(nib.Nifti1Image(pat_mask.astype(np.uint8), 
                              ct_nii.affine, ct_nii.header), pat_path)
    
    # 結果表示
    print("\n=== EAT/PAT抽出結果 ===")
    print(f"EAT体積: {eat_volume_ml:.2f} ml ({np.sum(eat_mask)} voxels)")
    print(f"PAT体積: {pat_volume_ml:.2f} ml ({np.sum(pat_mask)} voxels)")
    print(f"EAT保存先: {eat_path}")
    print(f"PAT保存先: {pat_path}")
    
    # 統計情報をテキストファイルに保存
    stats_path = os.path.join(output_dir, "eat_pat_stats.txt")
    with open(stats_path, 'w') as f:
        f.write("EAT/PAT Extraction Statistics\n")
        f.write("=" * 40 + "\n")
        f.write(f"Input CT: {ct_path}\n")
        f.write(f"Myocardium mask: {myocardium_path}\n")
        f.write(f"Voxel spacing: {spacing} mm\n")
        f.write(f"Voxel volume: {voxel_volume_ml:.6f} ml\n")
        f.write(f"Fat threshold: -190 to -30 HU\n")
        f.write(f"EAT dilation: 5mm ({dilation_5mm} steps)\n")
        f.write(f"PAT dilation: 10mm ({dilation_10mm} steps)\n")
        f.write(f"EAT volume: {eat_volume_ml:.2f} ml\n")
        f.write(f"PAT volume: {pat_volume_ml:.2f} ml\n")
    
    return eat_volume_ml, pat_volume_ml

def main():
    # デフォルトパス設定
    base_dir = "/home/soya/ctrate_ws"
    
    # 入力ファイル
    ct_path = f"{base_dir}/data/CT-RATE-v2/dataset/valid_fixed/valid_1/valid_1_a/valid_1_a_1.nii.gz"
    myocardium_path = f"{base_dir}/outputs/step3_heartchambers/heart_myocardium.nii.gz"
    output_dir = f"{base_dir}/outputs/step4_eat_pat"
    
    # コマンドライン引数処理
    if len(sys.argv) > 1:
        ct_path = sys.argv[1]
    if len(sys.argv) > 2:
        myocardium_path = sys.argv[2]
    if len(sys.argv) > 3:
        output_dir = sys.argv[3]
    
    print("=" * 50)
    print("STEP4: EAT/PAT抽出")
    print("=" * 50)
    
    # 実行
    try:
        eat_vol, pat_vol = extract_eat_pat(ct_path, myocardium_path, output_dir)
        print("\n✓ 正常終了")
        return 0
    except Exception as e:
        print(f"\n✗ エラー: {e}")
        import traceback
        traceback.print_exc()
        return 1

if __name__ == "__main__":
    sys.exit(main())