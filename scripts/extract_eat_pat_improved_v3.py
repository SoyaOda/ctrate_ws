...
def check_required_masks(totalseg_dir):
    ...
    search_dirs = [
        totalseg_dir,
        totalseg_dir.parent / "segmentation",
        totalseg_dir.parent / "segmentation_total"
    ]
    
    # 必須マスク
    required_masks = {
        'heart': ['heart_myocardium.nii.gz', 'myocardium.nii.gz', 'heart.nii.gz'],
        'lungs': ['lung_lower_lobe_left.nii.gz', 'lung_lower_lobe_right.nii.gz',
                 'lung_upper_lobe_left.nii.gz', 'lung_upper_lobe_right.nii.gz',
                 'lung_middle_lobe_right.nii.gz', 'lung_left.nii.gz', 'lung_right.nii.gz', 'lung.nii.gz'],
...
def load_lung_masks(totalseg_dir, verbose=False):
    ...
    for search_dir in search_dirs:
        ...
        for name in lobe_names:
            lobe_path = search_dir / f"{name}.nii.gz"
            if lobe_path.exists():
                lobes_found.append(lobe_path)
        
        if lobes_found:
            # ローブファイルが見つかった場合
            for lobe_path in lobes_found:
                ...
                if lungs is None:
                    ...
                    lungs |= data
                if verbose:
                    print(f"  Loaded: {lobe_path.stem}")
            return lungs
        
        # 左右の肺を探す
        lung_left_path = search_dir / "lung_left.nii.gz"
        lung_right_path = search_dir / "lung_right.nii.gz"
        lung_combined_path = search_dir / "lung.nii.gz"
        ...
        if lung_left_path.exists() and lung_right_path.exists():
            ...
            if verbose:
                print(f"  Loaded lung_left and lung_right from {search_dir}")
            return lungs
            ...
            if verbose:
                print(f"  Loaded lung (combined) from {search_dir}")
            return lungs
    
    # どのディレクトリでも見つからなかった場合
    raise FileNotFoundError(
...
def get_liver_first_z(totalseg_dir, verbose=False):
    """
    肝臓が最初に出現するZ座標（最も頭側）を取得
    
    Returns:
        z_liver_first: 肝臓が最初に出現するZ座標（見つからない場合はNone）
    """
    totalseg_dir = Path(totalseg_dir)
    ...
    liver_path = None
    for search_dir in search_dirs:
        test_path = search_dir / "liver.nii.gz"
        if test_path.exists():
            liver_path = test_path
            break
    
    if liver_path is None:
        if verbose:
            print("  WARNING: Liver mask not found")
        return None
    
    liver_img = nib.load(str(liver_path))
    liver_mask = liver_img.get_fdata() > 0
    
    # 肝臓が存在するZ座標を探す
    z_any = np.any(liver_mask, axis=(0,1))
    z_idx = np.where(z_any)[0]
    
    if z_idx.size > 0:
        z_liver_first = z_idx[-1]  # 最も頭側（大きいZ値）
        if verbose:
            print(f"  Liver first appears at Z={z_liver_first}")
        return z_liver_first
...
def load_visceral_fat_mask(totalseg_dir, verbose=False):
    ...
    for search_dir in search_dirs:
        torso_fat_path = search_dir / "torso_fat.nii.gz"
...
def extract_eat_pat_improved_v3(ct_path, totalseg_dir, output_dir, 
    ...
    # 7. 肝臓の最初の出現位置を取得
    print("\n7.5. Finding liver appearance level...")
    z_liver_first = get_liver_first_z(totalseg_dir, verbose)
    if z_liver_first is None:
        print("  WARNING: Liver not found, using standard Shell definition")
    ...
    # 10. 縦隔クランプ（胸腔内限定）- 肝臓レベルで処理を変更
    print("\n11. Applying mediastinal restriction (with liver-level adaptation)...")
    mediastinum = ~lungs
    
    # 肝臓レベルでの適応的Shell定義
    if z_liver_first is not None:
        print(f"  Adapting Shell at liver level (Z < {z_liver_first})...")
        
        # 各Zスライスで適応的にShellを定義
        shell_adaptive = np.zeros_like(shell_mask, dtype=bool)
        
        for z in range(shell_mask.shape[2]):
            if z < z_liver_first:
                # 肝臓レベルより尾側：心臓ShellまたはMediastinumの和集合
                shell_adaptive[:, :, z] = shell_mask[:, :, z] | mediastinum[:, :, z]
                if verbose and z == z_liver_first - 1:
                    print(f"    Below liver: Using Shell OR Mediastinum (union)")
            else:
                # 肝臓レベルより頭側：心臓ShellとMediastinumの積集合
                shell_adaptive[:, :, z] = shell_mask[:, :, z] & mediastinum[:, :, z]
        
        # 脂肪候補：適応的Shell内のtorso_fat
        fat_candidate = shell_adaptive & fat_clipped
        
        if verbose:
            original_shell_voxels = np.sum(shell_mask)
            adaptive_shell_voxels = np.sum(shell_adaptive)
            print(f"  Shell adapted: {original_shell_voxels:,} → {adaptive_shell_voxels:,} voxels")
        
        # Shellを適応版に更新
        shell_mask = shell_adaptive
    else:
        # 肝臓が見つからない場合は従来通り
