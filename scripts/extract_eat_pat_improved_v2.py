#!/usr/bin/env python3
"""
改善版v2 EAT+PAT抽出：処理順序を最適化
近位大血管ベースの下端Z → (~lungs)の縦隔クランプ → 腹部臓器クランプ → HU＋心膜ブリッジ
"""
...
def load_vessel_masks(totalseg_dir, verbose=False):
    ...
    # 別名の血管も試す（一部のバージョンでの命名差異）
    if "vena_cava_inferior" not in vessels:
        ivc_path = totalseg_dir / "inferior_vena_cava.nii.gz"
        if ivc_path.exists():
            if verbose:
                print(f"  Loading inferior_vena_cava (IVC)...")
            img = nib.load(str(ivc_path))
            vessels["vena_cava_inferior"] = img.get_fdata() > 0
...
def extract_eat_pat_improved_v2(ct_path, totalseg_dir, output_dir, 
                                dilation_radius_mm=DILATION_RADIUS_MM,
                                use_proximal_vessels=True,
                                verbose=False):
    """
    改善版v2 EAT+PAT（心膜周囲脂肪）抽出
    処理順序を最適化：近位大血管ベースの下端Z → (~lungs)の縦隔クランプ → 腹部臓器クランプ → HU＋心膜ブリッジ
    ...
    Returns:
        stats: 統計情報の辞書
    """
    print("\n[Improved EAT+PAT Extraction v2 - Optimized Processing Order]")
    print("="*60)
    ...
    # 4. 【重要】近位大血管ベースの下端・上端Z決定
    print("\n5. Computing anatomical boundaries (IVC/PV-based)...")
    ...
    # 5. Shell（心臓周囲領域）の計算（EDTベースの等方的膨張）
    print(f"\n6. Computing Shell (EDT-based dilation: {dilation_radius_mm} mm)...")
    ...
    # 6. 【重要】Zクリップを適用
    print("\n7. Applying Z-clipping to Shell and fat...")
    ...
    print(f"  Shell and fat clipped to anatomical boundaries")
    
    # 7. 【重要】縦隔クランプ（胸腔内限定）- 腹部臓器クランプより先
    print("\n8. Applying mediastinal clamping (thoracic cavity restriction)...")
    lungs = load_lung_masks(totalseg_dir, verbose)
    
    if lungs is not None:
        # 肺の外側（縦隔）に限定
        mediastinum = ~lungs
        fat_candidate = (shell_mask & fat_clipped) & mediastinum
        
        if verbose:
            initial_voxels = np.sum(shell_mask & fat_clipped)
            removed_by_lungs = initial_voxels - np.sum(fat_candidate)
            print(f"  Removed {removed_by_lungs:,} voxels outside mediastinum")
    else:
        fat_candidate = shell_mask & fat_clipped
        print("  No lung masks found for mediastinal clamping")
    
    # 8. 腹部臓器クランプ（偽陽性除去）- 縦隔クランプの後
    print("\n9. Applying abdominal organ clamping...")
    ...
    # 9. HUフォールバックと心膜ブリッジ（偽陰性救済）
    print("\n10. Applying HU threshold and pericardial bridge...")
    ...
    seed = fat_candidate | (bridge & hu_fat)
    propagation_mask = (shell_mask | bridge) & hu_fat
    
    # 連結成分の伝播
    reached = binary_propagation(seed, mask=propagation_mask, structure=struct)
    
    # 最終的なEAT+PATマスク（ブリッジ領域を除外）
    ...
    stats = {
        ...
        "parameters": {
            ...
            "processing_order": "Optimized v2",
            "improvements": [
                "IVC/PV-based inferior boundary",
                "Z-clipping before mediastinal restriction",
                "Mediastinal clamping before abdominal",
                "Abdominal organ clamping (3mm)",
                "HU thresholding (-190 to -30)",
                "Pericardial bridge (2mm)"
            ],
    ...
    print(f"\n12. Statistics saved: {stats_path}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Improved EAT+PAT extraction v2 with optimized processing order",
    ...
    # 実行
    try:
        stats = extract_eat_pat_improved_v2(
        ...
        print("\n" + "="*60)
        print("Improved EAT+PAT extraction v2 completed successfully!")
