#!/usr/bin/env python3
"""
改善版v5 EAT+PAT抽出：連結成分ベースILAM実装
lung_ROI_3.mdのフィードバックに基づく改善：
- EDT基準の等方膨張に統一
- 連結成分ラベル法で塊ごと採用
- Z方向の帯域計算を修正
"""
...
DEFAULT_LUNG_CONTACT_MM = 2.0    # 肺接触判定の許容距離 [mm] (δ) - v5.1で増加
DEFAULT_ABDOMEN_GAP_MM = 3.0     # 腹部臓器除外の膨張距離 [mm] (γ)
DEFAULT_MAX_HEART_DIST_MM = 35.0 # 心臓からの最大距離 [mm] (ρ) - v5.1で増加
...
def extract_eat_pat_improved_v5(ct_path, totalseg_dir, output_dir, 
    ...
    # 14. ILAM (Inferior Lung-Adjacency Mode) の適用 - 改善版
    print("\n14. Applying ILAM (Inferior Lung-Adjacency Mode) with Connected Components...")
    print(f"  Parameters:")
    ...
    print(f"  Seed voxels (lung-touching fat): {seed_voxels:,}")
    ...
    n_touched = len(touch_ids)
    print(f"  Components touching lungs: {n_touched}")
    
    # 触れた連結成分を全て採用
    ilam_components = np.isin(lbl, touch_ids)
    ilam_voxels = np.sum(ilam_components)
    print(f"  ILAM component voxels: {ilam_voxels:,}")
    ...
    stats = {
        ...
        "parameters": {
            ...
            "processing_version": "v5 with Connected Component ILAM",
            "ilam_settings": {
                ...
                "n_components_touched": int(n_touched) if 'n_touched' in locals() else 0
            },
            "improvements": [
                ...
                "ILAM v5: Connected Component Lung-Adjacency Mode",
...
def main():
    parser = argparse.ArgumentParser(
        description="Improved EAT+PAT extraction v5.1 with Full-Z Connected Component ILAM",
    ...
    try:
        ...
        stats = extract_eat_pat_improved_v5(
        ...
        print("\n" + "="*60)
        print("Improved EAT+PAT extraction v5 (Connected Component ILAM) completed successfully!")
