...
DEFAULT_LUNG_CONTACT_MM = 2.0    # 肺接触判定の許容距離 [mm] (δ)
...
def extract_eat_pat_improved_v5_2(ct_path, totalseg_dir, output_dir, 
                                     ...
                                     seal_diaphragm_holes_mm=0.0,
                                     verbose=False):
    """
    改善版v5.2 EAT+PAT（心膜周囲脂肪）抽出 with 優勢接触ILAM
    下部帯域で肺接触ベースの脂肪抽出を追加
    """
    ...
    # 13.5. 個別臓器マスクの読み込み（v5.2: 優勢接触判定用）
    ...
    # 14. ILAM (Inferior Lung-Adjacency Mode) v5.2 - 優勢接触ルール版
    print("\n14. Applying ILAM v5.2 (Dominant Contact Rule)...")
    print(f"  Parameters:")
    ...
    # ILAMのドメイン：no_z_limitなら全Z、そうでなければfat_clipped
    ...
    # Z帯域制限（no_z_limitでなければ適用）
    if not no_z_limit:
        zmask = np.zeros_like(domain, dtype=bool)
        zmask[:, :, z_min:z_switch+1] = True
        domain &= zmask
        print(f"  Domain restricted to Z band: {z_min} to {z_switch}")
    ...
    # 優勢接触を満たす成分だけを採用
    ilam_components = np.isin(lbl, np.array(keep_ids, dtype=lbl.dtype)) if keep_ids else np.zeros_like(domain)
    ilam_voxels = np.sum(ilam_components)
    print(f"  ILAM component voxels (after filtering): {ilam_voxels:,}")
    ...
    stats = {
        ...
        "parameters": {
            ...
            "processing_version": "v5.2 with Dominant Contact ILAM",
            "ilam_settings": {
...
def main():
    parser = argparse.ArgumentParser(
        description="Improved EAT+PAT extraction v5.2 with Dominant Contact ILAM",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="肺マスクの微小欠損を閉じる半径 [mm] (0で無効, default: 0.0)"
    )
    ...
    try:
        ...
        stats = extract_eat_pat_improved_v5_2(
        ...
        print("\n" + "="*60)
        print("Improved EAT+PAT extraction v5.2 (Dominant Contact Rule) completed successfully!")
