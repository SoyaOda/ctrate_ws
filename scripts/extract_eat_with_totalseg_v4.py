#!/usr/bin/env python3
"""
改善版v4：TotalSegmentatorとEDTを使用したEAT抽出
肺門漏れ対策強化版：
- 血管の近位化を厳格化（閾値を下げる）
- 遠位血管禁止帯でシェルを削る
- 気道近傍の禁止帯を追加
...
def build_adaptive_shell_with_connectivity(dist_mm, band20, heart_core, heart_all, 
    ...
    if far_vessel_mask.any():
        ...
        print(f"  Distal vessel exclusion: {before_anti:,} → {after_anti:,} voxels "
              f"(removed {before_anti-after_anti:,})")
    ...
    if airway_removed > 0:
        print(f"  Total airway exclusion: {airway_removed:,} voxels")
    ...
    if far_vessel_mask.any():
        ...
        print(f"[VALIDATION] Distal vessel proximity ratio: {ratio_far*100:.2f}% (expected < 2%)")
        if ratio_far > 0.02:
            print("[WARNING] High distal vessel proximity! Consider reducing KEEP_MM values.")
...
def save_results(eat_mask, shell, ct_img, output_dir, shell_thickness_mm,
    ...
    if 50 <= eat_vol_ml <= 200:
        print(f"  ✓ EAT volume ({eat_vol_ml:.2f} ml) is within expected range (50-200 ml)")
    else:
        print(f"  ⚠ EAT volume ({eat_vol_ml:.2f} ml) is outside typical range (50-200 ml)")
    
    if eat_mean_hu is not None and not np.isnan(eat_mean_hu):
        if hu_min <= eat_mean_hu <= hu_max:
            print(f"  ✓ EAT mean HU ({eat_mean_hu:.1f}) is within fat range")
        else:
            print(f"  ⚠ EAT mean HU ({eat_mean_hu:.1f}) is unusual for fat tissue")
...
def main():
    parser = argparse.ArgumentParser(description="改善版v4 EAT抽出（肺門漏れ対策強化）")
